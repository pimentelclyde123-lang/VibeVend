...
export default function VibeVend() {
  ...
  // -------- Performance mode detection (Android 7 / low mem) --------
  const performanceMode = useMemo(() => {
    const ua = (navigator.userAgent || '').toLowerCase();
    const isOldAndroid = /android 5|android 6|android 7/.test(ua);
    const deviceMem = navigator.deviceMemory || 2; // older WebViews don't expose this
  ...
  // Inject only the minimal keyframes we need (lighter animations)
  useEffect(() => {
  ...
  if (screen === 'home') {
    return (
      <div
        ...
        style={{
          // Dark nightclub base; if not performance mode, add gentle background motion
      ...
      >
        {/* Lightweight aura blobs (radial glows, very small blur, no transforms) */}
        <div className="absolute inset-0 pointer-events-none">
          <div
            ...
            style={{
              ...
              filter: performanceMode ? 'blur(10px)' : 'blur(28px)',
              opacity: 0.9,
            }}
          />
          <div
            ...
            style={{
              ...
              filter: performanceMode ? 'blur(8px)' : 'blur(24px)',
              opacity: 0.85,
            }}
          />
        </div>
        ...
        <div className="text-center z-10 max-w-2xl select-none">
          ...
          {/* Glow rings — simplified & cheaper to render */}
          <div className="relative mx-auto w-80 h-80">
            <div
              className="absolute inset-0 rounded-full border-[3px]"
              style={{
                borderColor: 'rgba(34,211,238,.55)',
                boxShadow: '0 0 28px rgba(34,211,238,.45), 0 0 60px rgba(34,211,238,.25)',
                animation: 'heartbeatLite 7s ease-in-out infinite',
            ...
            <div
              className="absolute inset-0 rounded-full border-[3px]"
              style={{
                borderColor: 'rgba(236,72,153,.55)',
                boxShadow: '0 0 28px rgba(236,72,153,.45), 0 0 60px rgba(236,72,153,.25)',
                animation: 'heartbeatLite 7s ease-in-out infinite',
                animationDelay: '2.2s',
                willChange: 'transform, opacity',
              }}
  ...
  if (screen === 'products') {
    const bgStyle = performanceMode
      ? {
          ...
          backgroundSize: '200% 200%', // smaller texture
          animation: 'none',
        }
      : {
          background:
